...
export default function CreateAccountWithValidation() {
  ...
  const handleCreateAccount = async () => {
    ...
    try {
      ...
      console.log("Making request to:", `${API_BASE_URL}/parent/login`);
      ...
      if (isSuccess) {
        ...
        if (token) {
          ...
          showCustomAlert(
            "success",
            "Login Successful",
            "Logged in successfully! Note: Some features may be limited.",
            false,
            () => {
              router.push("/ParentDashboard");
            }
      ...
      if (
        error.message &&
        error.message.toLowerCase().includes("login successful")
      ) {
      ...
      if (error instanceof Error) {
        if (error.message.includes("HTML instead of JSON")) {
          errorMessage =
            "Server configuration error. The login endpoint may not exist. Please contact support.";
